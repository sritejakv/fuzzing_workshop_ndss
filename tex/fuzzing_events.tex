...
\autoref{fig:fuzzing_loop_events} shows the corresponding internal data in grey boxes in \textbf{bold}.
...
\textit{M5. \totalenergyyield} that is computed as the number of \queueevents across different \energyevents in each fuzzing cycle.

\textit{M6. \covenergyyield} is computed as the number of \queueevents that are due to increase in coverage across different \energyevents.

\textit{M7. \hitenergyyield} is the number of \queueevents that are due to increase in edge hit count across different \energyevents.

\textit{M8. \zeroenergyyield} is computed as the number of \energyevents in the fuzzing loop that do not contain \queueevents, i.e., the number of wasted energy allocations by the underlying power schedule.

In the above evaluation metrics, we only consider two most likely interesting behaviors due to which a fuzzer adds a seed to the queue: coverage and hit count.
